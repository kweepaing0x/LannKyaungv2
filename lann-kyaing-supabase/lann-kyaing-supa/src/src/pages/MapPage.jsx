...
export default function MapPage() {
  ...
  const pickHandlerRef = useRef(null); // for location picking mode
  ...
  const {
    ...
    // location picking state (set by PlusModal)
          ...
          if (userMarkerRef.current) {
            ...
            const icon = L.divIcon({
              ...
              html:`<div style="position:relative;width:20px;height:20px">
                <div style="position:absolute;inset:-6px;border-radius:50%;border:2px solid rgba(74,158,255,0.4);animation:lkPulse 2s ease-out infinite"></div>
                <div style="width:20px;height:20px;background:#fff;border-radius:50%;border:3px solid #4a9eff;position:absolute;top:0;left:0"></div>
              </div>`,
  ...
  // PlusModal sets pickingLocation=true, user taps map → pickedLocation set
  useEffect(() => {
    ...
    if (pickHandlerRef.current) {
      ...
      mapRef.current?.classList.remove("picking-location");
    }

    if (pickingLocation) {
      mapRef.current?.classList.add("picking-location");
      const handler = (e) => {
    ...
    allPins.forEach(pin => {
      ...
      const icon  = L.divIcon({
        ...
        html:`<div style="width:${sz}px;height:${sz}px;background:${color};border-radius:50%;
          border:2px solid rgba(255,255,255,${hist?.3:.7});opacity:${hist?.35:1};
          box-shadow:0 0 ${hist?3:8}px ${color}${hist?"33":"88"}"></div>`,
        iconSize:[sz,sz], iconAnchor:[sz/2,sz/2],
      });
      const m = L.marker([pin.lat,pin.lng],{icon})
        .addTo(mapInstance.current)
        .bindPopup(`<div style="background:#1a1a1a;color:#fff;padding:12px 16px;border-radius:10px;text-align:center;font-family:sans-serif;min-width:120px">
    ...
    <div style={{position:"relative",width:"100%",height:"100%",background:"#0d0d0d"}}>
      ...
      {pickingLocation && (
        <div style={{
          ...
          background:"rgba(83,74,183,0.95)",padding:"12px 16px",
          display:"flex",alignItems:"center",justifyContent:"space-between",
        }}>
          <span style={{color:"#CECBF6",fontSize:13,fontWeight:600}}>
            Tap on the map to set location
          </span>
          <button onClick={()=>setPickingLocation(false)} style={{
            background:"none",border:"none",color:"#CECBF6",fontSize:13,cursor:"pointer"
          }}>Cancel</button>
        </div>
      )}
      ...
      {/* Right buttons */}
      <div style={{position:"absolute",top:60,right:12,display:"flex",flexDirection:"column",gap:6,zIndex:500}}>
        <button onClick={centerOnUser} style={rbtnStyle} aria-label="Center on my location">
