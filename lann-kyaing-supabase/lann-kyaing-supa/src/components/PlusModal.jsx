  ...
  async function handlePostPin(){
    ...
    try{
      ...
      await postPin({
        ...
        tipEnabled,
        tipAmount: tipEnabled ? TIP_AMOUNT : null,
      });
              ...
              <>
                ...
                <div style={{display:"flex",gap:8,marginBottom:14}}>
                  {[
                    ...
                    {id:"tip",  label:"☕ Tip enabled", desc:`Viewers can tip ${TIP_AMOUNT} pts`, icon:"☕"},
                  ].map(opt=>(
                ...
                {pinMode==="tip"&&(
                  <div style={{background:"rgba(239,159,39,0.08)",borderRadius:10,padding:"10px 12px",
                    ...
                    <div style={{color:"#888",fontSize:11,lineHeight:1.6}}>
                      Viewers can send you a ☕ tea tip of <strong style={{color:"#EF9F27"}}>{TIP_AMOUNT} pts</strong>.<br/>
                      After 20% commission you receive <strong style={{color:"#EF9F27"}}>{Math.round(TIP_AMOUNT*0.8)} pts</strong> per tip.
                    </div>
                  </div>
                )}
              </>
